...
func (s *AppstreamMetadataInterfaceSuite) TestMountConnectedPlug(c *C) {
	...
	c.Assert(os.MkdirAll(filepath.Join(tmpdir, "/usr/share/applications"), 0777), IsNil)
	...
	c.Check(entries[3].Name, Equals, filepath.Join(hostfs, dirs.GlobalRootDir, "/usr/share/applications"))
	c.Check(entries[3].Dir, Equals, "/usr/share/applications")
	...
	c.Check(entries[5].Name, Equals, filepath.Join(hostfs, dirs.GlobalRootDir, "/var/lib/app-info"))
	c.Check(entries[5].Dir, Equals, "/var/lib/app-info")
	c.Check(entries[5].Options, DeepEquals, []string{"bind", "ro"})
	c.Check(entries[6].Name, Equals, filepath.Join(hostfs, dirs.GlobalRootDir, "/var/lib/apt/lists"))
	c.Check(entries[6].Dir, Equals, "/var/lib/apt/lists")
